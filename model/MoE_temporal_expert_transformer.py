...
class Temporal_unit(nn.Module):
    def __init__(self, in_channels, out_channels, heads=3, residual=True, num_frames=100, depth=2, window_size=10,
                 ...
                 use_checkpoint=False, temporal_merge=False, poolDiffMerge=False):
        super().__init__()
        assert num_frames % window_size == 0, f"num_frames:{num_frames} can not divide window size: {window_size}"
        self.mlp_ratio = mlp_ratio
        self.ape = ape
        ...
        if temporal_merge and poolDiffMerge:
            merge_strategy = TemporalWindowPatchMergingPoolDiff

        if temporal_merge and not poolDiffMerge:
            merge_strategy = TemporalWindowPatchMerging
        ...
        self.trans = TemporalWindowTransformerLayer(
            ...
            norm_layer=norm_layer,
            downsample=merge_strategy if temporal_merge else None,
...
class TemporalWindowPatchMergingPoolDiff(nn.Module):
    r""" Patch Merging Layer.
    ...
    def forward(self, x):
        ...
        diff_x = x[:, :, 0::2] - x[:, :, 1::2]  # even minus odd
        x = torch.cat([pool_x, diff_x], dim=1)  # B L/2 2*C
...
class MoE_Temporal_Spatial_Trans_unit(nn.Module):
    def __init__(self, in_channels, out_channels, spatial_heads=3, temporal_heads=3, residual=True, dropout=0.1,
                 temporal_merge=False, t_expert_receptive_field=[4, 8, 16, 32], num_frames=100,
                 t_expert_depth=[2, 2, 2, 2],
                 spatial_depth=1, topk=1, temporal_part_size=32, use_zloss=0.01, use_switchloss=0.001,
                 spatial_mask=None,
                 temporal_ape=False,
                 poolDiffMerge=False):
        super().__init__()
        ...
        self.use_switchloss = use_switchloss
        for i in range(self.num_expert):
            self.t_experts.append(
                Temporal_unit(in_channels,
                              ...
                              ape=temporal_ape, depth=t_expert_depth[i], poolDiffMerge=poolDiffMerge))
        ...
        if not residual:
            self.residual = lambda x: 0
        elif temporal_merge == True and not poolDiffMerge:
            self.residual = TemporalWindowPatchMerging(in_channels)
        elif temporal_merge == True and poolDiffMerge:
            self.residual = TemporalWindowPatchMergingPoolDiff(in_channels)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_parts):
            ss = i * self.temporal_part_size
            tt = (i + 1) * self.temporal_part_size
            expert_input = tx[:, ss:tt, :]
            topk_experts, part_logits, part_probs, part_freqs = self.topk_gate(expert_input)
            if self.temporal_merge:
                expert_output = torch.zeros((B * V, self.temporal_part_size // 2, C * 2))
            else:
                expert_output = torch.zeros((B * V, self.temporal_part_size, C))
            expert_output = expert_output.cuda(x.device)
            for e in topk_experts:
                ret = self.t_experts[e](expert_input)
                expert_output += ret
            output.append(expert_output)
            logits.append(part_logits)
            probs.append(part_probs)
            freqs.append(part_freqs)
        logits = torch.cat(logits, dim=0)
        probs = torch.cat(probs, dim=0)
        freqs = torch.cat(freqs, dim=0)
        zloss = self.use_zloss * self.compute_zloss(logits)
        switchloss = self.use_switchloss * self.compute_switchloss(probs, freqs)
        atx = torch.cat(output, dim=1)
        ...
        stx = stx + rx
        MoE_loss = zloss + switchloss
        return self.relu(stx), MoE_loss
    ...
    def compute_switchloss(self, probs, freqs):
        ...
        return loss.sum() * self.num_expert
...
class myZiT(nn.Module):
    def __init__(self, in_channels=3, num_person=5, num_point=18, spatial_heads=6, temporal_heads=3, graph=None,
                 graph_args=dict(), num_frame=100, embed_dim=48,
                 t_expert_receptive_field=[4, 8, 16, 32],
                 t_expert_depth=[2, 2, 2, 2],
                 topk=1, temporal_part_size=32, use_zloss=0.01, use_switchloss=0.001, add_spatial_mask=False,
                 temporal_ape=False, poolDiffMerge=False
                 ):
        ...
        self.layers = nn.ModuleList()
        # self.layers.append(MoE_Temporal_Spatial_Trans_unit(in_channels=48, out_channels=48, spatial_heads=spatial_heads,
        #                                                    temporal_heads=temporal_heads, residual=True,
        #                                                    num_frames=num_frame,
        #                                                    t_expert_receptive_field=t_expert_receptive_field,
        #                                                    t_expert_depth=t_expert_depth,
        #                                                    topk=topk, temporal_part_size=temporal_part_size,
        #                                                    use_zloss=use_zloss,use_switchloss=use_switchloss,
        #                                                    spatial_mask=self.A if add_spatial_mask else None))
        self.layers.append(MoE_Temporal_Spatial_Trans_unit(in_channels=48, out_channels=96, spatial_heads=spatial_heads,
                                                           temporal_heads=temporal_heads, residual=True,
                                                           num_frames=num_frame,
                                                           t_expert_receptive_field=t_expert_receptive_field,
                                                           t_expert_depth=t_expert_depth,
                                                           ...
                                                           use_zloss=use_zloss, use_switchloss=use_switchloss,
                                                           spatial_mask=self.A if add_spatial_mask else None,
                                                           temporal_merge=True, poolDiffMerge=poolDiffMerge))
        # self.layers.append(MoE_Temporal_Spatial_Trans_unit(in_channels=96, out_channels=96, spatial_heads=spatial_heads,
        #                                                    temporal_heads=temporal_heads * 2, residual=True,
        #                                                    num_frames=num_frame // 2,
        #                                                    t_expert_receptive_field=[f // 2 for f in
        #                                                                              t_expert_receptive_field],
        #                                                    t_expert_depth=t_expert_depth,
        #                                                    topk=topk, temporal_part_size=temporal_part_size // 2,
        #                                                    use_zloss=use_zloss,use_switchloss=use_switchloss,
        #                                                    spatial_mask=self.A if add_spatial_mask else None))
        self.layers.append(
            MoE_Temporal_Spatial_Trans_unit(in_channels=96, out_channels=192, spatial_heads=spatial_heads,
                                            temporal_heads=temporal_heads * 2, residual=True,
                                            num_frames=num_frame // 2,
                                            t_expert_receptive_field=[f // 2 for f in t_expert_receptive_field],
                                            t_expert_depth=t_expert_depth,
                                            topk=topk, temporal_part_size=temporal_part_size // 2, use_zloss=use_zloss,
                                            use_switchloss=use_switchloss,
                                            spatial_mask=self.A if add_spatial_mask else None,
                                            temporal_merge=True, poolDiffMerge=poolDiffMerge))
        self.layers.append(
            MoE_Temporal_Spatial_Trans_unit(in_channels=192, out_channels=192, spatial_heads=spatial_heads,
                                            temporal_heads=temporal_heads * 4, residual=True,
                                            num_frames=num_frame // 4,
                                            t_expert_receptive_field=[f // 4 for f in t_expert_receptive_field],
                                            t_expert_depth=t_expert_depth,
                                            topk=topk, temporal_part_size=temporal_part_size // 4, use_zloss=use_zloss,
                                            use_switchloss=use_switchloss,
                                            spatial_mask=self.A if add_spatial_mask else None))
...
class ZoTTransformer(nn.Module):
    def __init__(self, num_class=15, spatial_heads=6, temporal_heads=3,
                 t_expert_receptive_field=[4, 8, 16, 32],
                 t_expert_depth=[2, 2, 2, 2],
                 topk=1, temporal_part_size=32, use_zloss=0.01, use_switchloss=0.001, spatial_depth=1):
        super().__init__()
        self.l1 = MoE_Temporal_Spatial_Trans_unit(in_channels=192, out_channels=192, spatial_heads=spatial_heads,
                                                  temporal_heads=temporal_heads, residual=True,
                                                  num_frames=64,
                                                  t_expert_receptive_field=t_expert_receptive_field,
                                                  t_expert_depth=t_expert_depth,
...
class Model(nn.Module):
    def __init__(self, num_class=15, in_channels=3, num_person=5, num_point=18, num_frame=128, s_num_head=6, graph=None,
                 graph_args=dict(), addMotion=False,
                 onlyXYZ=False, angularType="p2a", addSpatialMask=False, temporalApe=False,
                 t_expert_receptive_field=[4, 8, 16, 32], temporal_part_size=32, t_expert_depth=[2, 2, 2, 2], topK=2,
                 t_num_head=3, use_zloss=0.01, use_switchloss=0.001, ZoTType="org", poolDiffMerge=False):
        ...
        self.body_transf = myZiT(in_channels=in_channels, num_person=num_person, num_point=num_point,
                                 ...
                                 add_spatial_mask=addSpatialMask, t_expert_receptive_field=t_expert_receptive_field,
                                 temporal_part_size=temporal_part_size, t_expert_depth=t_expert_depth,
                                 temporal_ape=temporalApe, spatial_heads=s_num_head, temporal_heads=t_num_head,
                                 topk=topK, use_zloss=use_zloss, use_switchloss=use_switchloss,
                                 poolDiffMerge=poolDiffMerge)
        if ZoTType == "direct":
            self.group_transf = directFC(num_class=num_class)
        elif ZoTType == "org":
            self.group_transf = simpleZoT(num_class=num_class)
        elif ZoTType == "transformer":
            reduce_times = 4
            self.group_transf = ZoTTransformer(num_class=num_class, spatial_heads=s_num_head,
                                               temporal_heads=t_num_head,
                                               t_expert_receptive_field=[f // reduce_times for f in
                                                                         t_expert_receptive_field],
                                               t_expert_depth=t_expert_depth, topk=topK,
                                               temporal_part_size=temporal_part_size // reduce_times,
                                               use_zloss=use_zloss, use_switchloss=use_switchloss, spatial_depth=2)
    ...
    def forward(self, x):
        ...
        if isinstance(x, tuple):
            x, zloss2 = x
        return x, zloss + zloss2


if __name__ == '__main__':
    m = Model(angularType='ntu-60')
    dummy_input = torch.randn(1, 3, 256, 25, 2)
    x, zloss = m(dummy_input)
