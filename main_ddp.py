...
class Processor():
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train' and not self.arg.DDP:
        ...
        elif self.arg.phase == 'train' and self.arg.DDP:
    ...
    def load_model(self):
        ...
        if self.arg.DDP:
            device = torch.device("cuda", self.arg.local_rank)
            torch.cuda.set_device(self.arg.local_rank)
            dist.init_process_group(backend='nccl')
            self.model = self.model.to(device)
            self.model = DDP(self.model, device_ids=[self.arg.local_rank], output_device=self.arg.local_rank)
